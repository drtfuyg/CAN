...
ZCAN_PCIE_CANFD_800U    = ZCAN_DEVICE_TYPE(82)
ZCAN_PCIE_CANFD_1200U   = ZCAN_DEVICE_TYPE(83)
ZCAN_MINI_PCIE_CANFD    = ZCAN_DEVICE_TYPE(84)
ZCAN_USBCANFD_800H      = ZCAN_DEVICE_TYPE(85)
ZCAN_BG002 = ZCAN_DEVICE_TYPE(86)
ZCAN_BG004 = ZCAN_DEVICE_TYPE(87)
...
class ZCAN_DEVICE_INFO(Structure):
    ...
    def _version(self, version):
        if version &0x8000: #最高位为1，则表示版本号为三段式结构
            return "V%02x.%02x.%02x" % ((version&0x7fff) //0xfff, ((version&0x0fff) // 0xFF) % 0x100, version & 0xFF)#三段式
        return ("V%02x.%02x" if version // 0xFF >= 9 else "V%d.%02x") % (version // 0xFF, version & 0xFF) #两段式
...
class ZCAN_LIN_PUBLISH_CFG(Structure):
    _pack_ = 1
    _fields_ = [("ID", c_ubyte),            # 受保护的ID（ID取值范围为0-63）
...
# LIN 响应结构体
class ZCAN_LIN_PUBLISH_CFG_EX(Structure):
    _pack_ = 1
    _fields_ = [("ID", c_ubyte),  # ID
                ("dataLen", c_ubyte),  # dataLen范围为1-64
                ("data", c_ubyte * 64),
                ("chkSumMode", c_ubyte),  # 校验方式，0-默认，启动时配置 1-经典校验 2-增强校验
                ("reserved", c_ubyte * 5)]
...
# CANFD定时发送参数
class ZCAN_AUTO_TRANSMIT_OBJ_PARAM(Structure):  # auto_send delay
...
# LIN调度表相关
ZCAN_LIN_CHN_SCHED_MAX_NUM  = 128  # 通道支持添加最大调度表个数
ZCAN_LIN_SCHED_ITEM_MAX_NUM = 64   # 调度表项最大数

# LIN调度表状态
ZCAN_LIN_SCHED_STATUS_IDLE = 0  # 空闲
ZCAN_LIN_SCHED_STATUS_RUN  = 1  # 正在运行
ZCAN_LIN_SCHED_STATUS_ERR  = 2  # 运行出错

# LIN帧类型 ZCAN_LIN_FRAME_TYPE
...
LIN_SCHED_ALL_ITEMS = 0xFFFFFFFF  # 对应 ((UINT)(-1))

# flag 联合体
class LinSchedItemFlagUnion(Union):
    _fields_ = [
        ("disable", c_ubyte, 1),    # 1位
        ("have_data", c_ubyte, 1),  # 1位
        ("res", c_ubyte, 6),        # 6位
    ]

# ids 联合体：偶发帧 ID
class LinSporadicId(Structure):
    _fields_ = [
        ("spor_related_id", c_ubyte * 16),  # 偶发帧关联的id列表，索引0为最高优先级，依次递减
        ("spor_count", c_ubyte),            # 偶发帧的关联的id个数
    ]

# ids 联合体：事件触发帧 ID
class LinEventId(Structure):
    _fields_ = [
        ("event_id", c_ubyte),              # 事件触发帧的id
        ("event_related_id", c_ubyte * 16), # 事件触发帧关联的无条件帧id列表
        ("event_count", c_ubyte),           # 事件触发帧关联的无条件帧的id个数
    ]

# ids 联合体
class LinIdsUnion(Union):
    _fields_ = [
        ("id", c_ubyte),                # 无条件帧、诊断主机请求帧、诊断从机应答帧、保留帧的id
        ("sporadic_id", LinSporadicId), # 偶发帧
        ("event_id", LinEventId),       # 事件触发帧
    ]

# 调度表项
class ZCAN_LIN_SCHED_ITEM(Structure):
    _fields_ = [
        ("type", c_ubyte),                  # ZCAN_LIN_FRAME_TYPE
        ("flag_union", LinSchedItemFlagUnion),
        ("reserved1", c_ubyte * 2),         # 预留
        ("slot", c_uint),                   # 帧时隙(单位ms)
        ("resolve_sched_tbl_idx", c_uint),  # 冲突调度表索引
        ("ids", LinIdsUnion),
        ("reserved2", c_ubyte * 2),         # 预留
        ("data", c_ubyte * 8),              # 数据区
    ]

# flag联和体
class LinSchedTableFlagUnion(Structure):
    _fields_ = [
        ("disable", c_uint, 1),  # 1位：失能
        ("is_conflict", c_uint, 1),  # 1位：冲突表
        ("res", c_uint, 30),  # 30位：保留
    ]

# 调度表
class ZCAN_LIN_SCHED_TABLE(Structure):
    _fields_ = [
        ("run_cnt", c_uint),                    # 调度表运行次数
        ("flag_union", LinSchedTableFlagUnion), # 联和体
        ("items", ZCAN_LIN_SCHED_ITEM * ZCAN_LIN_SCHED_ITEM_MAX_NUM), # 调度表中帧信息数组
        ("item_num", c_uint),                   # 调度表中帧的个数
    ]
...
def ZCAN_DYNAMIC_CONFIG_CAN_ENABLE(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_ENABLE"

# 工作模式，默认正常模式；0：正常模式；1：只听模式
def ZCAN_DYNAMIC_CONFIG_CAN_MODE(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_MODE"

# 发送失败是否重传：0：发送失败不重传1：发送失败重传，直到总线关闭（CANFDNET- 100 / 200无此项配置）
def ZCAN_DYNAMIC_CONFIG_CAN_TXATTEMPTS(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_TXATTEMPTS"

# CAN波特率或CANFD仲裁域波特率
def ZCAN_DYNAMIC_CONFIG_CAN_NOMINALBAUD(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_NOMINALBAUD"

# CANFD数据域波特率
def ZCAN_DYNAMIC_CONFIG_CAN_DATABAUD(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_DATABAUD"

# 终端电阻开关；0：关闭；1：打开
def ZCAN_DYNAMIC_CONFIG_CAN_USERES(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_USERES"
...
# 报文发送间隔，0~255ms
def ZCAN_DYNAMIC_CONFIG_CAN_SNDCFG_INTERVAL(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_SNDCFG_INTERVAL"

# 总线利用率使能，使能后，将周期发送总线利用率到设定的TCP/UDP连接。1:使能，0：失能
def ZCAN_DYNAMIC_CONFIG_CAN_BUSRATIO_ENABLE(can_idx):
    return f"DYNAMIC_CONFIG_CAN{can_idx}_SNDCFG_INTERVAL"
...
class ZCAN(object):
    ...
    # 基于自定义序列号打开
    def OpenDeviceByName(self, device_type, name):
        try:
            self.__dll.ZCAN_OpenDeviceByName.argtypes = [c_uint32, c_char_p]
            return self.__dll.ZCAN_SetValue(device_type, name.encode("utf-8"))
        except:
            print("Exception on OpenDeviceByName!")
            raise
    ...
    # UDS请求 --- 仅支持 CAN(FD) 协议
    def UDS_Request(self, device_handle, req, resp, dataBuf, dataBufSize):
        try:
            return self.__dll.ZCAN_UDS_Request(device_handle, byref(req), byref(resp), dataBuf, dataBufSize)
        except:
            print("Exception on UDS_Request")
            raise

    # UDS 诊断控制，如停止正在执行的 UDS 请求 --- 仅支持 CAN(FD) 协议
    def UDS_Control(self, device_handle, ctrl, resp):
        try:
            return self.__dll.ZCAN_UDS_Control(device_handle, byref(ctrl), byref(resp))
        except:
            print("Exception on UDS_Control")
            raise

    # UDS请求 --- CAN LIN 都支持
    def UDS_RequestEx(self, device_handle, requestData, resp, dataBuf, dataBufSize):
        try:
            return self.__dll.ZCAN_UDS_RequestEX(device_handle, byref(requestData), byref(resp), dataBuf, dataBufSize)
        except:
            print("Exception on UDS_RequestEx")
            raise

    # UDS 诊断控制，如停止正在执行的 UDS 请求 --- CAN LIN 都支持
    def UDS_ControlEx(self, device_handle, dataType, ctrl, resp):
        try:
            return self.__dll.ZCAN_UDS_ControlEx(device_handle, dataType, byref(ctrl), byref(resp))
        except:
            print("Exception on UDS_ControlEx")
            raise

    # 设置LIN通道调度表
    def SetLINSchedule(self, chn_handle, sched_tbl, sched_tbl_num):
        try:
            return self.__dll.ZCAN_SetLINSchedule(chn_handle, byref(sched_tbl), sched_tbl_num)
        except:
            print("Exception on SetLINSchedule")
            raise

    # 设置调度表表项使能状态
    def SetLINScheduleItemEnabled(self, chn_handle, sched_tbl_idx, item_idx, enabled):
        try:
            return self.__dll.ZCAN_SetLINScheduleItemEnabled(chn_handle, sched_tbl_idx, item_idx, enabled)
        except:
            print("Exception on SetLINScheduleItemEnabled")
            raise

    # 获取LIN调度表状态信息
    def GetLINScheduleStatus(self, chn_handle, status):
        try:
            return self.__dll.ZCAN_GetLINScheduleStatus(chn_handle, byref(status))
        except:
            print("Exception on GetLINScheduleStatus")
            raise

    # 启动LIN通道调度表
    def StartLINSchedule(self, chn_handle, sched_tbl_idx):
        try:
            return self.__dll.ZCAN_StartLINSchedule(chn_handle, sched_tbl_idx)
        except:
